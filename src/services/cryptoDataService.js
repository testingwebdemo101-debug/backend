...
class CryptoDataService {
    ...
    async fetchFromCoinGecko() {
        try {
            ...
            console.log(`✓ CoinGecko: Processed ${processedCount} coins`);
    ...
    async getCoinChartData(coinSymbol, days = 7) {
        ...
        try {
            ...
            if (response.status === 429) {
                console.warn('⚠ Rate limited by CoinGecko for chart data');
            ...
            console.error(`❌ CoinGecko chart error for ${coinSymbol}:`, coinGeckoError.message);
    ...
    async getCoinDetail(coinSymbol) {
        ...
        try {
            ...
            if (response.status === 429) {
                console.warn('⚠ Rate limited by CoinGecko for detail');
                throw new Error('Rate limited');
            }
